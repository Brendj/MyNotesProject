...
<%-- Панель загрузки списка сотрудников --%>
<h:panelGrid id="newVisitorFileLoaderPanel" binding="#{mainPage.visitorDogmLoadPage.pageComponent}" styleClass="borderless-grid">
    <h:panelGrid columns="2" styleClass="borderless-grid">
        <h:outputText escape="true" value="Проверять дубликаты (по серии и номеру документов)"
                      styleClass="output-text" />
        <h:selectBooleanCheckbox value="#{mainPage.visitorDogmLoadPage.checkVisitorNoUnique}" styleClass="output-text">
            <a4j:support event="onclick" reRender="newVisitorFileLoaderPanel" ajaxSingle="true" />
        </h:selectBooleanCheckbox>
    </h:panelGrid>

    <rich:fileUpload id="newVisitorFileUploadElement" styleClass="upload" addButtonClass="upload-command-button"
                     ...
                     fileUploadListener="#{mainPage.newVisitorLoadFileListener}">
        <f:facet name="label">
            <h:outputText escape="true" value="{_KB}KB/{KB}KB [{mm}:{ss}]" />
        </f:facet>
        <a4j:support event="onuploadcomplete" reRender="newVisitorFileLoaderPanel" />
        <a4j:support event="onclear" reRender="visitorFileLoaderPanel" />
    </rich:fileUpload>

    <h:outputText escape="true"
                  value="Обработано: #{mainPage.visitorDogmLoadPage.lineResultSize}. Успешно: #{mainPage.visitorDogmLoadPage.successLineNumber}"
                  styleClass="output-text" />

    <rich:dataTable id="newVisitorLoadResultTable" value="#{mainPage.visitorDogmLoadPage.lineResults}" var="item" rows="20"
        ...
        <f:facet name="footer">
            <rich:datascroller for="newVisitorLoadResultTable" renderIfSinglePage="false" maxPages="5" fastControls="hide"
    ...
    <h:commandButton value="Выгрузить в CSV" action="#{mainPage.showNewVisitorLoadResultCSVList}"
