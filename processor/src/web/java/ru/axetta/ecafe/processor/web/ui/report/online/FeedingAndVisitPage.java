...
import ru.axetta.ecafe.processor.core.report.FeedingAndVisitReport;
...
@Component
@Scope("session")
public class FeedingAndVisitPage extends OnlineReportPage{

    private final static Logger logger = LoggerFactory.getLogger(FeedingAndVisitPage.class);
    ...
    public Object buildReportHTML() {
        ...
        String templateShortFileName = FeedingAndVisitReport.class.getSimpleName() + ".jasper";
        String templateFilename = autoReportGenerator.getReportsTemplateFilePath() + templateShortFileName;
        if(!(new File(templateFilename)).exists()){
            printError(String.format("Не найден файл шаблона '%s'", templateShortFileName));
            return null;
        }
        FeedingAndVisitReport.Builder builder = new FeedingAndVisitReport.Builder(templateFilename);
    ...
    public void showCSVList(ActionEvent actionEvent){
        ...
        try {
            ...
            Org org = (Org) session.load(Org.class, idOfOrg);
            builder.setOrg(new BasicReportJob.OrgShortItem(org.getIdOfOrg(), org.getShortName(), org.getOfficialName()));
            FeedingAndVisitReport feedingAndVisitReport = (FeedingAndVisitReport) builder.build(session,startDate, endDate, localCalendar);
            persistenceTransaction.commit();
            persistenceTransaction = null;
            FacesContext facesContext = FacesContext.getCurrentInstance();
            ...
            String filename = buildFileName(generateTime, feedingAndVisitReport);
            response.setHeader("Content-disposition", String.format("inline;filename=%s", filename));

            JRXlsExporter xlsExport = new JRXlsExporter();
            xlsExport.setParameter(JRCsvExporterParameter.JASPER_PRINT, feedingAndVisitReport.getPrint());
    ...
    private String buildFileName(Date generateTime, FeedingAndVisitReport feedingAndVisitReport) {
        DateFormat timeFormat = new SimpleDateFormat("dd.MM.yyyy-HH:mm:ss");
        String reportDistinctText = feedingAndVisitReport.getReportDistinctText();
        String format = timeFormat.format(generateTime);
        return String.format("%s-%s-%s.xls", "FeedingAndVisitReport", reportDistinctText, format);
    }
    ...
    public String getPageFilename() {
        return "report/online/feeding_and_visit_report";
    }
