 ...
 */
public class ActOfWayBillDifferencePositionFilter extends AbstractFilter {
    ...
    public boolean isEmpty() {
        return (StringUtils.isEmpty(number) && idOfOrg==null && startDate==null && endDate==null);
    }
    ...
    protected void apply(EntityManager entityManager, Criteria crit) {
        ...
        if (startDate!=null) {
            crit.add(Restrictions.ge("date", CalendarUtils.truncateToDayOfMonth(startDate)));
            if (endDate==null) endDate = CalendarUtils.addOneDay(startDate);
        }
        if (endDate!=null) crit.add(Restrictions.le("date", CalendarUtils.truncateToDayOfMonth(endDate)));
    }
