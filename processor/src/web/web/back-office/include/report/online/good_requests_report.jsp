...
<h:panelGrid id="goodRequestReportPanelGrid" binding="#{mainPage.goodRequestReportPage.pageComponent}"
    ...
    <rich:simpleTogglePanel label="Настройки отчета" switchType="client" style="width: 800px;" opened="true"
        ...
        <h:panelGrid styleClass="borderless-grid" columns="2">
            ...
            <h:panelGroup>
                <a4j:commandButton value="..." action="#{mainPage.goodRequestReportPage.showContragentListSelectPage}"
                                   reRender="modalOrgListSelectorPanel"
                                   oncomplete="if (#{facesContext.maximumSeverity == null}) #{rich:component('modalOrgListSelectorPanel')}.show();"
                                   styleClass="command-link" style="width: 25px;">
                    <f:setPropertyActionListener value="2" target="#{mainPage.orgListSelectPage.filterMode}" />
                    <f:setPropertyActionListener value="#{mainPage.goodRequestReportPage.contragentStringIdOfOrgList}"
                                                 target="#{mainPage.orgFilterOfSelectOrgListSelectPage}" />
                    <f:setPropertyActionListener value="Выбор контрагента" target="#{mainPage.orgFilterPageName}" />
                </a4j:commandButton>
                <h:outputText styleClass="output-text" escape="true"
                              value=" {#{mainPage.goodRequestReportPage.contragentFilter}}" />
            </h:panelGroup>

            <h:outputText styleClass="output-text" escape="true" value="Организация" />
            <h:panelGroup>
                <a4j:commandButton value="..." action="#{mainPage.goodRequestReportPage.showOrgListSelectPage}"
                                   reRender="modalOrgListSelectorPanel"
                                   oncomplete="if (#{facesContext.maximumSeverity == null}) #{rich:component('modalOrgListSelectorPanel')}.show();"
                                   styleClass="command-link" style="width: 25px;">
                    <f:setPropertyActionListener value="1" target="#{mainPage.orgListSelectPage.filterMode}" />
                    <f:setPropertyActionListener value="#{mainPage.goodRequestReportPage.getStringIdOfOrgList}"
                                                 target="#{mainPage.orgFilterOfSelectOrgListSelectPage}" />
                </a4j:commandButton>
                <h:outputText styleClass="output-text" escape="true"
                              value=" {#{mainPage.goodRequestReportPage.filter}}" />
            </h:panelGroup>
            ...
            <rich:calendar value="#{mainPage.goodRequestReportPage.startDate}" datePattern="dd.MM.yyyy"
                ...
                <a4j:support event="onchanged" reRender="endDateCalendar"
                             actionListener="#{mainPage.goodRequestReportPage.onReportPeriodChanged}" />
            </rich:calendar>

            <h:outputText styleClass="output-text" escape="true" value="Интервал выборки" />
            <h:selectOneMenu id="endDatePeriodSelect"
                             value="#{mainPage.goodRequestReportPage.periodTypeMenu.periodType}" styleClass="input-text"
                             style="width: 250px;">
                <f:converter converterId="periodTypeConverter" />
                <f:selectItems value="#{mainPage.goodRequestReportPage.periodTypeMenu.items}" />
            ...
            <h:selectBooleanCheckbox value="#{mainPage.goodRequestReportPage.hideMissedColumns}"
            ...
            <h:selectOneMenu value="#{mainPage.goodRequestReportPage.orgsFilter}" converter="javax.faces.Integer"
                ...
                <f:selectItem itemValue="-1"
                              itemLabel="Отображать организации с отсутствием заявок за последние 2 дня" />
            </h:selectOneMenu>

            <h:outputText escape="true" value="Суточная проба" styleClass="output-text" />
            <h:selectBooleanCheckbox value="#{mainPage.goodRequestReportPage.showDailySamplesCount}"
                                     styleClass="output-text" />
            <h:selectOneMenu value="#{mainPage.goodRequestReportPage.dailySamplesMode}" rendered="false"
                             styleClass="output-text">
            ...
            <h:commandButton value="Генерировать отчет в Excel" actionListener="#{mainPage.exportGoodRequestReport}"
    ...
    <h:panelGrid styleClass="borderless-grid">
        ...
        <c:if test="${not empty mainPage.goodRequestReportPage.goodRequestsReport && not empty mainPage.goodRequestReportPage.goodRequestsReport.goodRequestItems}">
            ...
            <rich:dataTable id="goodRequestsReportTable"
                            value="#{mainPage.goodRequestReportPage.goodRequestsReport.goodRequestItems}" var="req"
