    ...
    static
        ...
        TYPES.put ("BenefitPartChart",
                new SimpleType ("select \'Льготные категории 1-4 класс\', count(cf_clients.idofclient) " +
                                ...
                                new Object [][] { { ValueType.TEXT, "Льготные категории по питанию в общем составе учащихся" },
                                                  { ValueType.NUMBER, "Количество учащихся" } },
        ...
        TYPES.put ("BenefitsChart",
                new SimpleType ("select cf_categorydiscounts.categoryname, count (cat) " +
                                ...
                                new Object [][] { { ValueType.TEXT, "Детализация льготных категорий кроме 1-4 класса" },
                                                  { ValueType.NUMBER, "Количество учащихся" } },
                                BENEFITS_CHART_DATA));
        TYPES.put ("VisitorsChart",
                    new ComplexType (new Type [] { new SimpleType ("select '' || EXTRACT(EPOCH FROM d) * 1000 as date, " +
                                                   ...
                                                   new SimpleType ("select '' || EXTRACT(EPOCH FROM d) * 1000 as date, " +
                                                                   ...
                                                                   "group by events.d, overall.o, overall.c " +
                                                                   "having cast(count(events.c) as float8)/cast(overall.c as float8) > 0.2) as res " +
                                                                   "group by d", VISITORS_CHART_2_DATA).setIncremental (true) },
