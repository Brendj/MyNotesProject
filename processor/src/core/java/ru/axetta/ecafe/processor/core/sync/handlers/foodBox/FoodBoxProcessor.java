...
public class FoodBoxProcessor extends AbstractProcessor<OrgSettingSection> {

    private final FoodBoxRequest foodBoxRequest;
    private Long idOfOrg;
    private static final Logger logger = LoggerFactory.getLogger(FoodBoxProcessor.class);

    public FoodBoxProcessor(Session session, FoodBoxRequest foodBoxRequest, Long idOfOrg) {
        super(session);
        this.foodBoxRequest = foodBoxRequest;
        this.idOfOrg = idOfOrg;
    }

    @Override
    public ResFoodBoxRemain process() throws Exception {
        Long maxVersionFromARM = foodBoxRequest.getMaxVersion();
        //Apply change from ARM
        foodBoxRequest.getItems();
        return  null;
