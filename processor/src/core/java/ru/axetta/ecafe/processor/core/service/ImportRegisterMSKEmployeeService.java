...
@Scope("singleton")
public class ImportRegisterMSKEmployeeService extends ImportRegisterMSKClientsService {

    private static final org.slf4j.Logger logger = LoggerFactory.getLogger(ImportRegisterMSKEmployeeService.class);
    ...
    public StringBuffer syncEmployeesWithRegistry(long idOfOrg, StringBuffer logBuffer) throws Exception {
        ...
        ImportRegisterMSKClientsService.OrgRegistryGUIDInfo orgGuids = new ImportRegisterMSKClientsService.OrgRegistryGUIDInfo(org);
        logger.info(synchDate + "Производится синхронизация сотрудников для " + org.getOfficialName() + " GUID [" + orgGuids.getGuidInfo()
                + "]", logBuffer);

        List<ImportRegisterMSKClientsService.ExpandedPupilInfo> pupils = getPupilsByOrgGUID(orgGuids.orgGuids, null, null, null);
    ...
    public List<ImportRegisterMSKClientsService.ExpandedPupilInfo> getPupilsByOrgGUID(Set<String> orgGuids,
