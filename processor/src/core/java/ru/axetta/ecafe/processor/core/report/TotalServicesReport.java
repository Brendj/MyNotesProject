...
public class TotalServicesReport extends BasicReport
{
    ...
    {
        public TotalServicesReport build (Session session, Date startDate,
                Date endDate, List<Long> idOfOrgList) throws Exception
        {
            ...
            String preparedQuery = "select cf_orgs.idoforg, cf_orgs.officialname, count(distinct cf_clients.idofclient) " +
                    "from cf_orgs " +
                    "left join cf_clients on cf_clients.idoforg = cf_orgs.idoforg " +
                    (orgCondition.length () > 0 ? "where " + orgCondition : "") +
            ...
            loadValue (entries, "currentClientsCount", session,
                    "select cf_enterevents.idoforg, count(distinct cf_enterevents.idofclient) " +
                            "from cf_enterevents " +
                            "where cf_enterevents.evtdatetime between EXTRACT(EPOCH FROM TIMESTAMP '" + df.format (startDate) + "') * 1000 AND " +
        ...
        private void loadValue (Map <Long, TotalEntry> entries, String valueKey,
                Session session, String preparedQuery)
        {
            Query query = session.createSQLQuery (preparedQuery);
            List resultList = query.list ();
