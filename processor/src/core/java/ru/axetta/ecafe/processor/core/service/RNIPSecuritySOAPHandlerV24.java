...
public class RNIPSecuritySOAPHandlerV24 extends RNIPSecuritySOAPHandlerV22 {
    private static final Logger logger = LoggerFactory.getLogger(RNIPSecuritySOAPHandlerV24.class);

    public RNIPSecuritySOAPHandlerV24(String containerAlias, String containerPassword, IRNIPMessageToLog messageLogger) {
    ...
    private void replaceNamespaceVersion(Element nodeItem) {
        ...
        for (int i = 0; i < map.getLength(); i++) {
            ...
            if (node.getLocalName().startsWith("ns")) {
                node.setValue(node.getValue().replace("2.1.1", "2.4.0"));
            }
        }
    }

    private Node replaceNamespaceVersion(Document doc, Node nodeItem) {
        return doc.renameNode(nodeItem, nodeItem.getNamespaceURI().replace("2.1.1", "2.4.0"), nodeItem.getNodeName());
    }
    ...
    private Node replaceNamespaceVersionInverse(Document doc, Node nodeItem) {
        return doc.renameNode(nodeItem, nodeItem.getNamespaceURI().replace("2.4.0", "2.1.1"), nodeItem.getNodeName());
    }
}
