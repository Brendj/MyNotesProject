    ...
    */
    ...
    protected static final List<List<ReferReportItem>> getTotalItems(int workDaysCount, int weekendsCount,
        ...
        Query q = session.createSQLQuery(
                ...
                + "       from ("
                + "             select idoforg, subcategory, count(distinct idofclient) as cnt, ordday "
                + "             from ("
                + "                   select cl.idoforg, dr.subcategory, ord.idofclient, date_trunc('day', to_timestamp(ord.orderdate / 1000)) as ordday "
                + "                   from cf_orgs o "
                + "                   left join cf_orders ord on o.idoforg=ord.idoforg "
                + "                   join cf_clients cl on ord.idofclient=cl.idofclient and cl.idoforg=o.idoforg "
                + "                   join cf_clientscomplexdiscounts ccd on ccd.idofclientcomplexdiscount= "
                + "                        (select ccd2.idofclientcomplexdiscount from cf_clientscomplexdiscounts ccd2 where cl.idofclient=ccd2.idofclient order by createdate desc limit 1) "
                + "                   join cf_discountrules dr on ccd.idofrule=dr.idofrule "
                + "                   where cl.idofclientgroup<1100000000 and subcategory<>'' and ord.socdiscount<>0 and ord.rsum=0 "
                +                           regionClause + orgClause
                + "                         and ord.orderdate>=:startDate and ord.orderdate<:endDate "
                + "                   ) as aa "
                + "             group by idoforg, subcategory, ordday "
                + "             ) as bb "
