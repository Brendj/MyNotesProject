 ...
 */
public class ReestrTaloonApprovalData implements AbstractToElement {
    private List<ResTaloonApprovalItem> items;

    public ReestrTaloonApprovalData() {

    }

    @Override
    public Element toElement(Document document) throws Exception {
        Element element = document.createElement("ReestrTaloonApproval");
        for (ResTaloonApprovalItem item : this.getItems()) {
            element.appendChild(item.toElement(document, "RTA"));
        }
        return element;
    }
