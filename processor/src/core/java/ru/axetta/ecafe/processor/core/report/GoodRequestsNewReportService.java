...
public class GoodRequestsNewReportService {
    ...
    public GoodRequestsNewReportService(Session session, long OVERALL, long OVERALL_TOTAL, String OVERALL_TOTAL_TITLE,
            String OVERALL_TITLE, boolean hideTotalRow) {
    ...
    public List<Item> buildRepotItems(Date startTime, Date endTime, String nameFilter, int orgFilter,
        ...
        for (Object obj : orgList) {
            ...
            BasicReportJob.OrgShortItem educationItem = new BasicReportJob.OrgShortItem(idOfOrg, row[1].toString(),
                    row[2].toString());
            if (row[3] != null) {
        ...
        for (Object obj : goodRequestPositionList) {
            ...
            if (StringUtils.isEmpty(name)) {
                name = position.getGood().getNameOfGood();
            }
            ...
            for (BasicReportJob.OrgShortItem item : orgMap.values()) {
                if (item.getSourceMenuOrg() != null) {
                    ...
                    for (Object name : fullNameProviderMap.getCollection(item.getSourceMenuOrg())) {
                        if (hideMissedColumns) {
                            for (Date date : dates) {
                                addItemsFromList(itemList, item, date, name.toString(), hideDailySampleValue,
                                        hideLastValue);
                            }
                        } else {
                            ...
                            while (beginDate.getTime() <= endDate.getTime()) {
                                addItemsFromList(itemList, item, beginDate, name.toString(), hideDailySampleValue,
                                        hideLastValue);
                                beginDate = CalendarUtils.addOneDay(beginDate);
                            }
                        }
        ...
        if (itemList.isEmpty() && !hideTotalRow) {
            ...
            itemList.add(new Item(OVERALL, OVERALL_TITLE, "", CalendarUtils.truncateToDayOfMonth(startTime),
                    hideDailySampleValue, hideLastValue));
            itemList.add(new Item(OVERALL_TOTAL, OVERALL_TOTAL_TITLE, "", CalendarUtils.truncateToDayOfMonth(startTime),
                    hideDailySampleValue, hideLastValue));
        }
        return itemList;
    }
    ...
    private void addItemsFromList(List<Item> itemList, BasicReportJob.OrgShortItem org, Date doneDate, String name,
        ...
        if (!hideTotalRow) {
            ...
            itemList.add(
                    new Item(OVERALL_TOTAL, OVERALL_TOTAL_TITLE, "", doneDate, hideDailySampleValue, hideLastValue));
        }
    }
    ...
    private void addItemsFromList(List<Item> itemList, BasicReportJob.OrgShortItem org, Date doneDate, String name,
        ...
        if (!hideTotalRow) {
            itemList.add(new Item(OVERALL, OVERALL_TITLE, name, doneDate, totalCount, dailySampleCount, newTotalCount,
                    newDailySample, hideDailySampleValue, hideLastValue));
