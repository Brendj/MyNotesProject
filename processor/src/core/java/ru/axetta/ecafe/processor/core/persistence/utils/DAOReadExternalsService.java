...
public class DAOReadExternalsService {
    ...
    * */
    ...
    public Set<Long> getDishesRepeatable(WtComplex wtComplex, boolean usedforDeleted) {
        Set<Long> result = new HashSet<>();
        if (!wtComplex.getComposite()) return result;
        Query query;
        if (usedforDeleted)
            query = entityManager.createNativeQuery("select t.idofdish from cf_wt_complexes_dishes_repeatable t "
    ...
    public List<WtDish> getWtDishesByComplexItemAndDates(WtComplexesItem complexItem, Date startDate, Date endDate, boolean usedforDeleted) {
