...
public class MoneyInWords {
    ...
    private static final String leword[][] = {
            {"копейка", "копейки", "копеек", "0"}, {"рубль", "рубля", "рублей", "1"},
            {"тысяча", "тысячи", "тысяч", "0"}, {"миллион", "миллиона", "миллионов", "1"},
            {"миллиард", "миллиарда", "миллиардов", "1"}, {"триллион", "триллиона", "триллионов", "1"}};
    ...
    private static String num2words(long num, int level) {
        ...
        switch (n) {
            ...
            default:
                if ((h != 0) || ((h == 0) && (level
                        == 1)))  //если трехзначный сегмент = 0, то добавлять нужно только "рублей"
                {
    ...
    public static String inwords(double money) {
        ...
        int iw;
        switch (Integer.parseInt(skop.substring(1))) {
            case 1:
                iw = 0;
                break;
            case 2:
        ...
        long kop = Long.valueOf(skop);
        if (num < 1000000000000000l) {
            String result = num2words(num, 1) + " " + num2words(kop, 0);
            if (kop == 0) {
                result = result + " " + leword[0][iw];
            }
            return result;
        } else {
            return "error: слишком много рублей " + num2words(kop, 0) + " " + leword[0][iw];
        }
    }
}
