...
-- Processor.getAccRegistry(...) - Добавлен неоьязательный параметр параметр список ид клиентов, для получения информации карт толькол по выбранным клиента, в случаее пестого значения списка будет выведен весь список карт организации.
