...
--Для сервиса ЭЖД
CREATE OR REPLACE VIEW public.cf_ezd_request
    AS SELECT row_number() OVER () AS id,
       ...
       FROM ( SELECT DISTINCT co.idoforg,
              ...
              FROM cf_orgs co
                  ...
                  LEFT JOIN cf_clientgroups cg ON cg.idoforg = co.idoforg AND cg.groupname::text = cgto.groupname::text
...
CREATE OR REPLACE VIEW public.cf_ezd_request_new
    AS SELECT row_number() OVER () AS id,
           datas.idoforg,
           datas.guid,
           datas.ekisid,
       ...
       FROM ( SELECT DISTINCT co.idoforg,
              ...
              WHERE co.state = 1 AND co.organizationtype <> 2 AND co.preorderlp IS TRUE AND co.guid IS NOT NULL AND cci.modefree = 1 AND cci.menudate > (round(date_part('epoch'::text, now())) * 1000::double precision)::bigint
              ORDER BY co.guid, co.ekisid, cci.menudate) datas;

CREATE OR REPLACE VIEW public.cf_ezd_request_special_date
    AS SELECT row_number() OVER () AS id,
           resulted.date,
           resulted.groupname,
           resulted.idoforg,
       ...
       FROM ( SELECT DISTINCT sda.date,
              ...
              FROM cf_specialdates sda
                  ...
                  LEFT JOIN cf_clientgroups cg ON cg.idoforg = co.idoforg AND cg.groupname::text = cgto.groupname::text
                  LEFT JOIN cf_clients cc ON cc.idofclientgroup = cg.idofclientgroup AND cc.idoforg = co.idoforg
              WHERE co.state = 1 AND co.organizationtype <> 2 AND co.preorderlp IS TRUE AND sda.idoforg IS NOT NULL AND sda.deleted = 0 AND sda.date > (round(date_part('epoch'::text, now())) * 1000::double precision)::bigint AND cc.idofclient IS NOT NULL
